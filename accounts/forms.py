...
from .validators import allow_only_image

class UserForm(forms.ModelForm):
    password = forms.CharField(widget=forms.PasswordInput())
    confirm_password = forms.CharField(widget=forms.PasswordInput())
   # document_ID = forms.FileField(required=True)  # First document upload field
   # document_address = forms.FileField(required=True)  
    class Meta:
        model = User
